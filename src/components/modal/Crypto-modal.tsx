...
const CryptoModal: React.FC<CryptoModalProps> = ({
  ...
  return (
    <div className="w-full max-w-lg bg-white rounded-lg shadow-md p-6">
      ...
      <p className="text-gray-600 mb-6">Choose your preferred payment method to enroll in this course.</p>
      ...
      <div className="border border-gray-200 rounded-lg p-4 mb-6">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="font-bold text-lg mt-1 text-black">${price.toFixed(2)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center items-center mb-6">
        <div className="flex w-full rounded-md bg-gray-50 p-1">
          ...
          >
            <span className={currentMethod === 'crypto' ? 'text-black' : 'text-gray-500'}>
              <FaRegStar size={20} />
            </span>
            <span
              className={`font-medium ${currentMethod === 'crypto' ? 'text-black' : 'text-gray-500'
                }`}
            >
              Crypto
            </span>
          ...
          >
            <span className={currentMethod === 'card' ? 'text-blue-600' : 'text-gray-500'}>
              <FiCreditCard size={20} />
            </span>
            <span className={`font-medium ${currentMethod === 'card' ? 'text-blue-600' : 'text-gray-500'}`}>Card</span>
          </button>
        </div>
      </div>
      ...
      <div className="border border-gray-200 rounded-lg p-4 mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-bold text-lg text-black">Pay with Stellar (XLM)</h3>
          <span className="ml-auto font-bold text-black">{cryptoPrice} XLM</span>
        </div>
        <p className="text-gray-600">Connect your Stellar wallet to make a secure payment</p>
      </div>
      ...
      <p className="text-center text-gray-600 mb-1">Payments are processed on the Stellar blockchain</p>
      <p className="text-center text-gray-600 mb-6">Current rate: 1 XLM ≈ ${exchangeRate}</p>
