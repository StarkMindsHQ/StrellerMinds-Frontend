...
> = ({
  ...
  const { status, receipt, error, isConfirmed, isFailed, isLoading } =
    useTransactionStatus({
      hash: txHash,
      onConfirmed,
      onFailed,
  ...
  const getExplorerUrl = (hash: string) => {
    const baseUrl =
      (chainConfig as any)[chainId]?.explorer || 'https://etherscan.io';
            ...
            <div className="flex items-start gap-4">
              ...
              <div
                className={cn(
                  ...
                  status === 'confirmed'
                    ? 'bg-emerald-50/50 dark:bg-emerald-500/10 text-emerald-600'
                    : status === 'failed' || status === 'timeout'
                      ? 'bg-rose-50/50 dark:bg-rose-500/10 text-rose-600'
