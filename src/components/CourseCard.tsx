...
export function CourseCard({
  ...
  const getCardClasses = () => {
    const baseClasses = "group flex flex-col overflow-hidden rounded-xl border bg-white shadow-sm transition-all duration-300 ease-in-out hover:shadow-lg dark:border-gray-800 dark:bg-gray-950"
    
    switch (variant) {
      case "featured":
        return cn(baseClasses, "h-full min-h-[420px] hover:scale-[1.02] hover:shadow-xl")
      case "compact":
    ...
    <div className={cn(getCardClasses(), className)}>
      ...
      <div className="relative flex-shrink-0">
        <div className="absolute right-3 top-3 z-10">
          <span className={`inline-flex rounded-full px-3 py-1.5 text-xs font-semibold ${getBadgeStyles(level)}`}>
            {level}
          </span>
        </div>
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        {/* Features List */}
        {showFeatures && displayFeatures.length > 0 && variant !== "compact" && (
          <div className="mb-4">
