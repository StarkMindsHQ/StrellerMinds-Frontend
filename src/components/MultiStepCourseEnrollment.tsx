...
export const MultiStepCourseEnrollment: React.FC<
  MultiStepCourseEnrollmentProps
> = ({ steps, onSubmit, onCancel, defaultValues = {} }) => {
  const [currentStep, setCurrentStep] = useState(0);

  // Combine all schemas for a unified form or use 'any' schema to bypass full validation initially
  const combinedSchema = steps.reduce((acc, step) => {
    if (step.schema) {
          ...
          {steps.map((st, i) => {
            ...
            return (
              <div key={st.id} className="mb-4 last:mb-0">
                <h5 className="font-semibold text-foreground mb-2 pb-1 border-b">
                  {st.title}
                </h5>
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
                  {st.fields.map((f) => {
                    if (f.type === 'summary' || f.type === 'custom')
                      return null;
                    return (
                      <div key={f.name}>
