...
import { 
  PlayCircle, FileText, CheckCircle2, Download, 
  ChevronRight, Clock, BookOpen, AlertCircle 
} from 'lucide-react';
    ...
    return (
      <div className="bg-background min-h-screen">
        <div className="max-w-4xl mx-auto py-12 px-6">
          ...
          <h1 className="text-4xl font-extrabold mb-6 tracking-tight">{lesson.title}</h1>
            ...
            <div className="aspect-video bg-black rounded-3xl overflow-hidden shadow-2xl mb-12 group relative cursor-pointer ring-1 ring-white/10">
              ...
              <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between">
                <div className="text-white">
                  ...
                  <p className="text-lg font-bold">Watch Chapter 1: Introduction</p>
                </div>
          ...
          <div className="prose prose-lg prose-primary max-w-none dark:prose-invert">
            <div dangerouslySetInnerHTML={{ __html: lesson.content.richText?.html || '' }} />
          </div>
            ...
            <div className="mt-16 p-8 bg-muted/30 rounded-3xl border shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {lesson.attachments.map(att => (
                  <div key={att.id} className="p-4 bg-background border rounded-2xl flex items-center justify-between hover:border-primary transition-all group cursor-pointer shadow-sm">
      ...
      <div className="bg-muted/10 min-h-screen py-20 px-6">
        <div className="max-w-3xl mx-auto">
          <div className="bg-card border rounded-[40px] p-12 shadow-2xl shadow-primary/5 text-center relative overflow-hidden">
            ...
            <h1 className="text-4xl font-black mb-4 tracking-tighter">{quiz.title}</h1>
            <p className="text-muted-foreground text-lg mb-10 max-w-md mx-auto leading-relaxed">{quiz.description || "Test your knowledge and earn your certificate."}</p>
            
            <div className="grid grid-cols-2 gap-4 mb-10">
              <div className="p-6 bg-muted/30 rounded-3xl text-left border">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Pass Score</p>
                <p className="text-2xl font-black">{quiz.passingScore}%</p>
              </div>
              <div className="p-6 bg-muted/30 rounded-3xl text-left border">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Time Limit</p>
                <p className="text-2xl font-black">{quiz.settings.timeLimit || 'None'}</p>
              </div>
            </div>

            <Button size="lg" className="h-16 px-12 rounded-full text-lg font-black tracking-tight group">
              Start Final Quiz
            ...
            <div className="mt-12 pt-8 border-t flex items-center justify-center gap-8 text-xs font-black uppercase tracking-widest text-muted-foreground">
              <span className="flex items-center gap-2"><CheckCircle2 size={14} className="text-primary" /> {quiz.questions.length} Questions</span>
              <span className="flex items-center gap-2"><AlertCircle size={14} className="text-primary" /> {quiz.settings.attemptsAllowed} Attempts</span>
            </div>
          </div>
        </div>
      </div>
