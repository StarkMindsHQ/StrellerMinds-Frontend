  ...
  describe('GET /api/leaderboard/students', () => {
    ...
    it('should handle pagination with offset', async () => {
      const request1 = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?limit=5&offset=0',
        ),
      );
      const request2 = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?limit=5&offset=5',
        ),
      );
    ...
    it('should filter by metric type', async () => {
      const request = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?metricType=quiz_score',
        ),
      );
      ...
      expect(
        data.entries.every((e: any) => e.metricType === 'quiz_score'),
      ).toBe(true);
    });
    ...
    it('should reject invalid metric types', async () => {
      const request = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?metricType=invalid',
        ),
      );
    ...
    it('should filter by course ID', async () => {
      const request = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?courseId=course-1',
        ),
      );
      ...
      expect(data.entries.every((e: any) => e.courseId === 'course-1')).toBe(
        true,
      );
    });
    ...
    it('should include current user entry when userId provided', async () => {
      const request = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?userId=student-5',
        ),
      );
    ...
    it('should return undefined currentUserEntry when userId not found', async () => {
      const request = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?userId=nonexistent',
        ),
      );
    ...
    it('should calculate combined score correctly', async () => {
      const request = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?metricType=combined',
        ),
      );
    ...
    it('should set hasMore flag correctly', async () => {
      const requestFullPage = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?limit=10&offset=0',
        ),
      );
      const requestLastPage = new NextRequest(
        new URL(
          'http://localhost:3000/api/leaderboard/students?limit=10&offset=40',
        ),
      );
